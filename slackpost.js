...
module.exports = function (RED) {
  ...
  function SlackConfig(n) {
    ...
    this.webClient = null;
    this.rtmClient = null;
    const checkSubstring = inputString => /{global\.get\('.*?'\)}/.test(inputString);
    if (checkSubstring(this.credentials.token)) {
      const match = this.credentials.token.match(/{global\.get\('([^']+)'\)}/);
      const extractedSubstring = match ? match[1] : null;
      var globalToken = this.context().global.get(extractedSubstring);
      this.credentials.token = globalToken
    }
    this.shortToken = function () {
      if (this.credentials && this.credentials.hasOwnProperty("token")) {
        var token = this.credentials.token ? this.credentials.token : "";
        return token.substr(0, 8) + "..." + token.substr(token.length - 3);
      }
    };
    if (this.credentials && this.credentials.hasOwnProperty("token")) {
      console.log(this.credentials, "this.credentialsthis.credentials")
      /**
       ...
       */
      var tokenType = null;
      if (this.credentials.token && this.credentials.token.startsWith("xoxp-")) {
        tokenType = "user";
      } else if (this.credentials.token && this.credentials.token.startsWith("xoxb-")) {
        tokenType = "bot";
      } else {
